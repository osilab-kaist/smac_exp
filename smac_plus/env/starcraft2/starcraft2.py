...
from smac_plus.env.multiagentenv import MultiAgentEnv
from smac_plus.env.starcraft2.maps import get_map_params
...
class StarCraft2Env_Plus(MultiAgentEnv):
    ...
    def __init__(
        ...
        map_params = get_map_params(self.map_name, self.type)
    ...
    def _launch(self):
        ...
        ### added
        self.max_distance = self.distance(0,0,self.max_distance_x,self.max_distance_y)
    ...
    def step(self, actions):
        ...
        # 이 부분을 이런식으로 돌렸습니다! 아마 이렇게 해도 돌아갈겁니다!
        info["position"] = {}
        for al_id, al_unit in self.agents.items():
            if not self.death_tracker_ally[al_id]:
                # did not die so far
                info["position"][al_id] = [al_unit.pos.x, al_unit.pos.y, al_unit.pos.z]


        if self.reward_alt_ratio != 0:
            alt_reward = self.reward_alt_battle() / 20
            reward = self.reward_alt_ratio*alt_reward + (1-self.reward_alt_ratio)*reward
    ...
    def reward_alt_battle(self):
        ...
        for al_id, al_unit in self.agents.items():
            if not self.death_tracker_ally[al_id]:
                ...
                agent_reward = 0
                enemy_count = 0
                prev_pos = self.previous_ally_units[al_id].pos
                for e_id, e_unit in self.enemies.items():
                    if not self.death_tracker_enemy[e_id]:
                        enemy_count += 1
                        prev_e_pos = self.previous_enemy_units[e_id].pos
                        prev_dist = self.distance(prev_pos.x, prev_pos.y, prev_e_pos.x, prev_e_pos.y)
                        cur_dist = self.distance(al_unit.pos.x, al_unit.pos.y, e_unit.pos.x, e_unit.pos.y)
                        if self.reward_only_positive:
                            agent_reward += max(0,self.reward_negative_scale*(cur_dist-prev_dist))
                        else:
                            agent_reward += self.reward_negative_scale*(cur_dist-prev_dist)
                if enemy_count != 0 :
                    reward += agent_reward / enemy_count
                else:
                    reward += agent_reward
...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        obs_instead_of_state=False,
######################################
	    obs_instead_of_state_plus_enemy_center = False,
######################################
        obs_timestep_number=False,
        state_last_action=True,
        state_timestep_number=False,
        ...
        self.obs_instead_of_state = obs_instead_of_state
	    #######################################################
        self.obs_instead_of_state_plus_enemy_center = obs_instead_of_state_plus_enemy_center
	    #######################################################
        self.obs_last_action = obs_last_action
        self.obs_pathing_grid = obs_pathing_grid
        self.obs_terrain_height = obs_terrain_height
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (
                    ...
                    ind = 4
                    if self.obs_all_health:
                        enemy_feats[e_id, ind] = (           # 5 - 헬스 상태
                            e_unit.health / e_unit.health_max
                        )  # health
                        ind += 1
                        if self.shield_bits_enemy > 0:    #적군 실드가 들어가는 타입이면
                            max_shield = self.unit_max_shield(e_unit)
                            enemy_feats[e_id, ind] = (    # 6 - 실드 상태
                                e_unit.shield / max_shield
                            )  # shield
                            ind += 1
...
#########################################################################
    def get_obs_agent_for_state(self, agent_id):
        """Returns observation for agent_id. The observation is composed of:
        ...
        move_feats_dim = self.get_obs_move_feats_size()
        enemy_feats_dim = self.get_obs_enemy_feats_size_for_state()
        ally_feats_dim = self.get_obs_ally_feats_size()
        own_feats_dim = self.get_obs_own_feats_size()
        ...
        for e_id, e_unit in self.enemies.items():
            if e_unit.health > 0:
                ...
                coordinate_x.append((x - center_x) / self.max_distance_x)
                coordinate_y.append((y - center_y) / self.max_distance_y)
        
        X = np.mean(np.array(coordinate_x))
        Y = np.mean(np.array(coordinate_y))
        coor = np.array(X, Y)
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (
                    ...
                    ####################################################################
                    if self.obs_instead_of_state_plus_enemy_center:
                        enemy_feats[e_id, 4] = X
                        enemy_feats[e_id, 5] = Y
...
###############################################
        ...
        agents_obs = [self.get_obs_agent_for_state(i) for i in range(self.n_agents)]
        return agents_obs
###############################################
    ...
    def get_state(self):
        ...
        #############################################
        if self.obs_instead_of_state_plus_enemy_center:
            obs_concat = np.concatenate(self.get_obs_for_state(), axis=0).astype(
                np.float32
	    )
            return obs_concat
        ##############################################
    ...
    def get_state_dict(self):
        ...
        ########################################
    def get_obs_enemy_feats_size_for_state(self):
        """Returns the dimensions of the matrix containing enemy features.
        Size is n_enemies x n_features.
        """
        nf_en = 6 + self.unit_type_bits
...
#############################################################
    def get_obs_size_for_state(self):
        """Returns the size of the observation."""
        own_feats = self.get_obs_own_feats_size()
        move_feats = self.get_obs_move_feats_size()

        n_enemies, n_enemy_feats = self.get_obs_enemy_feats_size_for_state()
        n_allies, n_ally_feats = self.get_obs_ally_feats_size()
    ...
    def get_state_size(self):
        ...
        ##################################################
        if self.obs_instead_of_state_plus_enemy_center:
            return (self.get_obs_size_for_state()) * self.n_agents
        ################################################
